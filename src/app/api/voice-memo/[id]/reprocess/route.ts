...
export async function POST(
  ...
  try {
    ...
    const updatePayload = {
      ...
      // For now, assume the unified edge function (parse-artifact) will pick up items with ai_parsing_status = 'pending'
      ...
      // The user needs to ensure their parse-artifact edge function is invoked.
    ...
    console.log(`Attempting to invoke parse-artifact Edge Function for artifact ${artifactId}`);
    const { data: functionData, error: functionError } = await supabase.functions.invoke(
      'parse-artifact', // Name of the Edge Function
      { body: { artifactId: artifactId } } // Pass artifactId in the body
    );

    if (functionError) {
      console.error('Error invoking parse-artifact Edge Function:', functionError);
      ...
      console.log('Successfully invoked parse-artifact Edge Function. Response:', functionData);
    }
